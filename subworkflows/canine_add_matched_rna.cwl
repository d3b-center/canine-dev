...
steps:
  dict_to_contig_lengths:
    run: ../tools/dict_to_contig_lengths.cwl
    in:
  ...
  bedtools_slop:
    ...
    in:
      ...
      genome: dict_to_contig_lengths/output
      left:
        valueFrom: $(100)
      right:
        valueFrom: $(100)
  ...
  echo_temp_rna_header_full:
    run: ../tools/echo_temp_rna_header_full.cwl
    in: []
    out: [output]
  ...
  awk_temp_rna_vcf:
    run: ../tools/awk_temp_rna_vcf.cwl
    in:
      input_counts: bcftools_query_counts/output
      input_header: echo_temp_rna_header_full/output
    out: [output]
  
  bcftools_view_index:
    run: ../tools/bcftools_view_index.cwl 
    in:
      input_vcf: awk_temp_rna_vcf/output
      output_filename:
