...
inputs:
  ...
  chromosome:
    ...
    inputBinding:
      position: 2
      prefix: "--chromosome"
# Probably best handled by scatter and instance stacking individual jobs.
#  parallel:
#    type: 'int?'
#    doc: |
#      Defines the number of chromosomes to run in parallel. The output will be
#      divided in multiple files, one for each chromosome. The file name will be
#      composed by the output argument (used as prefix) and a chromosome name given by
#      the chromosome argument list. This imply that both output and chromosome
#      argument need to be set correctly
#    inputBinding:
#      position: 2
#      prefix: "--parallel"
