...
outputs:
  tmb_metrics_txt: { type: 'File', outputSource: coyote_tumor_mutation_burden/output }
  tmb_metrics_json: { type: 'File', outputSource: coyote_stats2json/output }
...
steps:
  ...
  coyote_tumor_mutation_burden:
    run: ../tools/coyote_tumor_mutation_burden.cwl
    in:
      bed:
  ...
  coyote_stats2json:
    run: ../tools/coyote_stats2json.cwl
    in:
      statfile: coyote_tumor_mutation_burden/output
      output:
        source: output_basename
