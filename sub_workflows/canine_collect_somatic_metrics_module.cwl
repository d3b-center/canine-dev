...
inputs:
  ...
  input_snpeff_vcf: { type: 'File?', doc: "VCF file, annotated by SnpEff, from which to collect metrics." }
  input_vep_vcf: { type: 'File?', doc: "VCF file, annotated by VEP, from which to collect metrics." }
  ...
  exome_flag: { type: 'boolean?', doc: "Set to true if sample is from an exome." }
