...
inputs:
  ...
  mappability_track: { type: 'File', doc: "BED file containing the mappability track (e.g. k100.umap.no_header.bed)." }
  ...
  exome: { type: 'boolean', doc: "Set to true for exome samples" }
  config_sex:
    type:
      - type: enum
        name: config_sex
...
outputs:
  ...
  recentered_cr_seg: { type: 'File', outputSource: coyote_annot_seg/output }
...
steps:
  ...
  gatk_intervallisttools:
    ...
    when: $(!inputs.disable_tool)
    in:
      disable_tool:
        source: exome
        valueFrom: $(!self)
      input_intervals: [normal_target_intervals, tumor_target_intervals]
      action:
        valueFrom: "INTERSECT"
      output_prefix:
  ...
  gatk_preprocessintervals:
    ...
    in:
      ...
      interval_merging_rule:
        valueFrom: "interval_merging_rule"
    out: [output]
